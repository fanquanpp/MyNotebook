# MySQL 数据库教程 | MySQL Database Tutorial

> @Author: fanquanpp
> @Version: v3.5.0
> @Category: MySQL Overview
> @Description: MySQL 完整教程：概述与环境、SQL基础、进阶查询、控制器与应用、数据类型、索引、事务与锁、多表联查、SQL注入。| Complete MySQL tutorial covering overview & setup, SQL basics, advanced queries, controllers & applications, data types, indexes, transactions & locks, multi-table joins, SQL injection.

> **路径**：`10-MySQL/` | **MySQL**：最流行的开源关系型数据库管理系统，广泛应用于Web应用开发。

## 1. 简介 | Introduction

MySQL 是由瑞典 MySQL AB 公司开发的开源关系型数据库管理系统，现在属于 Oracle 公司。它是最流行的数据库之一，被广泛应用于 Web 应用开发（如 WordPress、Facebook 早期版本）、企业级应用等领域。

## 2. 目录索引 | Directory Index

### 快速查阅 | Quick Reference

- [V10_101-MySQL名词注释查阅表.md](./V10_101-MySQL名词注释查阅表.md)
- [V10_102-MySQL快速查阅.md](./V10_102-MySQL快速查阅.md)

### 基础篇 | Basics

- [C10_101-概述与环境.md](./C10_101-概述与环境.md)
- [C10_102-SQL基础语法.md](./C10_102-SQL基础语法.md)
- [C10_103-进阶查询与联查.md](./C10_103-进阶查询与联查.md)
- [C10_104-控制器与应用.md](./C10_104-控制器与应用.md)
- [C10_105-数据类型与约束.md](./C10_105-数据类型与约束.md)
- [C10_106-索引与执行计划.md](./C10_106-索引与执行计划.md)

### 进阶篇 | Advanced

- [G10_201-索引与优化.md](./G10_201-索引与优化.md)
- [G10_202-事务与锁.md](./G10_202-事务与锁.md)
- [G10_203-多表联查详解.md](./G10_203-多表联查详解.md)
- [G10_301-SQL注入.md](./G10_301-SQL注入.md)

## 3. 更新日志 | Changelog

- **2026-05-03**
  - 修复编码问题，确保 UTF-8 无 BOM 格式
  - 统一 README 结构和版本号

- **2026-04-18**
  - 完成 MySQL 模块 3.0 结构优化规划，统一文件命名规范，优化目录结构

- **2026-04-06**
  - 深度优化 README.md 文件，完善结构和内容

- **2026-04-05**
  - 初版发布，建立子文档与索引结构
