# GitHub 一站式中文教程 | GitHub Platform Tutorial

> @Author: fanquanpp
> @Version: v3.5.0
> @Category: GitHub Overview
> @Description: GitHub 平台完整教程：账户安全、远程协议、仓库管理、分支策略、PR/Issue、Actions、Pages、协作规范与排错。 | Complete GitHub platform tutorial covering security, protocols, repositories, branching, PR/Issue, Actions, Pages, and collaboration.

> **路径**：`01-Github完整教程/` | **GitHub（Git 托管平台）**：网页端协作与 **CI/CD（持续集成/持续交付）**。

## 1. 简介 | Introduction

GitHub 平台完整教程：账户安全、远程协议、仓库管理、分支策略、PR/Issue、Actions、Pages、协作规范与排错。本教程与仓库内的 [07-Git](../07-Git/README.md) 形成互补关系，后者主要讲解 Git 命令与核心概念，而本教程则聚焦于 GitHub 平台的实战应用。

## 2. 目录索引 | Directory Index

### 快速查阅 | Quick Reference

- [V01_101-GitHub名词注释查阅表.md](./V01_101-GitHub名词注释查阅表.md)

### 基础篇 | Basics

- [C01_101-账户与安全.md](./C01_101-账户与安全.md)
- [C01_102-SSH与HTTPS.md](./C01_102-SSH与HTTPS.md)
- [C01_103-仓库创建与管理.md](./C01_103-仓库创建与管理.md)

### 进阶篇 | Advanced

- [G01_201-分支保护规则.md](./G01_201-分支保护规则.md)
- [G01_202-PullRequest协作流程.md](./G01_202-PullRequest协作流程.md)
- [G01_203-Issues与里程碑.md](./G01_203-Issues与里程碑.md)
- [G01_204-Actions与CI.md](./G01_204-Actions与CI.md)
- [G01_205-GitHubPages.md](./G01_205-GitHubPages.md)
- [G01_206-协作规范.md](./G01_206-协作规范.md)
- [G01_207-常见问题与排查.md](./G01_207-常见问题与排查.md)
- [G01_208-GitHub_Actions与Pages.md](./G01_208-GitHub_Actions与Pages.md)

## 3. 基础篇详细内容 | Basics Details

### 3.1 延伸阅读 | Further Reading

- [GitHub 官方文档](https://docs.github.com/)
- [Git 官方文档](https://git-scm.com/doc)
- [GitHub 技能认证](https://skills.github.com/)

## 4. 延伸阅读 | Further Reading

- [GitHub 官方文档](https://docs.github.com/)
- 本仓库：[07-Git](../07-Git/README.md)

## 5. 更新日志 | Changelog

- **2026-05-03**
  - 优化 README 结构，删除重复内容，补充快速查阅索引
  - 删除基础篇概述和核心知识点部分
  - 修复转义错误

- **2026-04-18**
  - 完成 GitHub 仓库 3.0 结构优化规划，统一文件命名规范，优化目录结构

- **2026-04-06**
  - 深度优化 README.md 文件，完善结构和内容

- **2026-04-05**
  - 初版发布，建立十篇子文档与索引结构
