...
核心思路都一样：先定义初始状态，hover时改变属性，transition负责平滑过渡
...
typeof 判断类型（注意两个坑）
  typeof null       返回 "object"  坑！历史bug
  typeof []         返回 "object"  坑！数组也是object

判断数组：Array.isArray([1,2,3])  返回 true
...
声明和访问
  索引从0开始
  fruits.length 获取长度
  fruits[fruits.length - 1] 获取最后一个

常用方法
  push / pop      末尾添加/删除
  unshift / shift  开头添加/删除
  indexOf / includes  查找
  splice / slice  删除/截取（splice修改原数组，slice不修改）
  join            数组转字符串
...
对象 -- 键值对的集合
  访问：obj.name 或 obj["age"]
  修改：obj.age = 21
  添加：obj.grade = "大二"（不存在的key自动创建）
  删除：delete obj.major
  遍历：for (let key in obj)
  解构赋值：const { name, age } = obj
  展开运算符：const copy = { ...obj }
  合并：const merged = { ...obj, gpa: 3.8 }
...
参数默认值：不传就用默认值，传了就用传的值
解构参数：传对象进去，自动解构，没传的属性用默认值
不传参要设默认值：最外层 = {} 防止不传参数时报错
...
找元素：
  querySelector("#id")       按id找
  querySelector(".class")    按class找
  querySelectorAll("li")     找所有li

改内容：
  textContent = "文字"       改纯文本
  innerHTML = "<b>HTML</b>"  改HTML

改class：
  classList.add("active")     加class
  classList.remove("active")  删class
  classList.toggle("active")  切换class
