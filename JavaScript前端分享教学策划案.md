...
## 二、HTML快速复习 -- Markdown vs HTML（5分钟）
...
常用标签：
  ...
  斜体：    <em>斜体</em>  或  <i>斜体</i>
  删除线：  <del>删除线</del>  或  <s>删除线</s>
  链接：    <a href="url">链接文字</a>
  图片：    <img src="图片url" alt="描述">
  列表：    <ul><li>无序项</li></ul>
            <ol><li>有序项</li></ol>
  代码：    <code>行内代码</code>  <pre>代码块</pre>
...
> HTML大家也学过了，快速过一下。标签成对出现，有开始有结束。常用标签就那些，标题h1到h6，段落p，加粗strong，链接a，图片img，列表ul/ol。一个HTML文档就是html包head和body，head放标题和元信息，body放页面内容。

---

### 2.3 Markdown vs HTML 语法对比

### PPT页面

```
Markdown vs HTML -- 同样的效果，不同的写法
...
> 把Markdown和HTML的语法放一起看就很清楚了。同样的效果，Markdown几个符号搞定，HTML要写完整的标签。比如加粗，Markdown两个星号，HTML要写strong开始标签和结束标签。链接更明显，Markdown方括号圆括号，HTML要写a标签加href属性。
...
HTML 适合：
  ...
  ✅ 需要交互（表单、按钮、JS操作）
  ✅ 需要嵌入视频、音频、Canvas
  ✅ 邮件模板（邮件只认HTML不认Markdown）
  特点：精确控制，功能完整，配合CSS/JS做任何事

它们的关系：
  Markdown →（解析器转换）→ HTML →（浏览器渲染）→ 网页

  你写的.md文件，GitHub/博客平台会自动转成HTML再显示
  所以Markdown能做的HTML都能做，反过来不行

实际开发中的选择：
  写文档 → Markdown（快、简洁）
  写网页 → HTML（精确、完整）
  写邮件模板 → HTML（邮件客户端只认HTML）
  写笔记 → Markdown（专注内容）
```

### 说的话

> 那什么时候用哪个？简单说：写文档用Markdown，写网页用HTML。
>
> Markdown的优势是快。写README、写技术笔记、写API文档，用Markdown几个符号就搞定格式，不用操心标签闭合、缩进对齐，专注在内容上。GitHub、博客平台、Notion这些工具都原生支持Markdown。
>
> HTML的优势是精确和完整。做网页必须用HTML，因为浏览器只认HTML。需要精确控制布局、样式、交互的时候，HTML配合CSS和JS什么都能做。还有邮件模板，邮件客户端只认HTML，Markdown不行。
>
> 它们的关系要搞清楚：Markdown最终会被转换成HTML。你在GitHub上写的README是Markdown，但GitHub显示的时候是先转成HTML再渲染的。所以Markdown能做的HTML都能做，反过来不行——HTML有表单、视频、Canvas这些Markdown没有的东西。
>
> 实际开发中，两个都要会。写文档用Markdown提效率，写网页用HTML做产品，不冲突。
...
> 今天内容不少，总结一下。开头聊了Markdown和HTML的关系，写文档用Markdown，写网页用HTML，Markdown最终会转成HTML。CSS部分重点讲了常用属性和卡片动画效果，Flex和Grid两种布局方式，还有四个常用界面模板。JS部分讲了var的坑、数据类型、数组对象、==和===、函数三种写法的this区别，还有九九乘法表、三角形、计算器三个实战例子。
>
...
|------|--------|
| 3分钟 | 开场结束 |
| 8分钟 | HTML介绍（Markdown vs HTML）讲完 |
| 20分钟 | CSS属性+卡片动画讲完 |
| 37分钟 | CSS布局+模板讲完 |
