<!--
文件名称：GitHub 一站式中文教程索引
原名称：无（新建）
用途描述：汇总 GitHub（Git 托管平台）账户安全、远程协议、仓库生命周期、分支策略、PR（Pull Request）/Issue、Actions、Pages、协作规范与排错；面向零基础到进阶读者；建议先阅读 04-Git版本控制 基础篇；关联 SUMMARY.md、笔记库务/术语表.md。
创建/更新：2026-04-06
-->

# GitHub 一站式中文教程 | GitHub Platform Tutorial

> **路径**：`00-Github完整教程/` | **GitHub（Git 托管平台）**：网页端协作与 **CI/CD（持续集成/持续交付）**。

## 1. 项目简介 | Introduction

本教程是 fanquanpp 个人综合学习笔记库中的 GitHub 平台教程部分，专注于 GitHub 网页端操作与平台特有能力，包括账户安全、远程协议、仓库生命周期、分支策略、PR/Issue、Actions、Pages、协作规范与排错等内容。本教程与仓库内的 [`04-Git版本控制`](../04-Git版本控制/README.md) 形成互补关系，后者主要讲解 Git 命令与核心概念，而本教程则聚焦于 GitHub 平台的实战应用。

### 教程定位

- **GitHub 平台指南**：专注于 GitHub 网页端操作和平台特有功能
- **实战导向**：提供详细的操作步骤和最佳实践，帮助读者快速上手
- **进阶提升**：从零基础到高级 CI/CD 配置，满足不同层次的学习需求
- **问题解决**：包含常见问题与排查指南，帮助读者解决实际使用中遇到的问题

**使用说明：**

- 本教程已开放为公共资源，允许他人访问和克隆
- 禁止直接修改本仓库内容
- 他人使用本教程内容时出现的任何问题与作者无关

## 2. 目录索引 | Directory Index

### 基础篇 | Basics
- [01-账户与安全.md](./基础/01-账户与安全.md)
- [02-SSH与HTTPS.md](./基础/02-SSH与HTTPS.md)
- [03-仓库创建与管理.md](./基础/03-仓库创建与管理.md)
- [基础篇 README](./基础/README.md)

### 进阶篇 | Advanced
- [04-分支保护规则.md](./进阶/04-分支保护规则.md)
- [05-PullRequest协作流程.md](./进阶/05-PullRequest协作流程.md)
- [06-Issues与里程碑.md](./进阶/06-Issues与里程碑.md)
- [07-Actions与CI.md](./进阶/07-Actions与CI.md)
- [08-GitHubPages.md](./进阶/08-GitHubPages.md)
- [09-协作规范.md](./进阶/09-协作规范.md)
- [10-常见问题与排查.md](./进阶/10-常见问题与排查.md)

## 3. 内容特色 | Key Features

- **全面覆盖**：从账户安全到高级 CI/CD 配置，涵盖 GitHub 平台的各个方面
- **结构清晰**：按照基础篇和进阶篇组织内容，便于读者循序渐进地学习
- **实用性强**：提供详细的操作步骤和代码示例，便于读者实际应用
- **双语注释**：关键概念和术语提供中英文对照，便于理解和记忆
- **图文并茂**：各子文档内含截图占位区块，便于后续补充图文教程

## 4. 阅读建议 | Reading Guide

1. 建议先阅读 [`04-Git版本控制`](../04-Git版本控制/README.md) 的基础内容，了解 Git 的核心概念和基本命令
2. 按照基础篇到进阶篇的顺序学习，循序渐进
3. 结合实际操作练习，加深对 GitHub 平台功能的理解
4. 遇到问题时参考「常见问题与排查」部分

## 5. 延伸阅读 | Further Reading

- [GitHub 官方文档](https://docs.github.com/) <!-- nofollow -->
- 本仓库：[04-Git版本控制](../04-Git版本控制/README.md)、[术语表](../meta/笔记库务/术语表.md)

## 6. 联系方式 | Contact Information

- 邮箱：<fanquanpangpiing@163.com>
- QQ：1839243393
- 欢迎提意见交流或反馈问题

## 7. 更新日志 | Changelog

- **2026-04-06**：深度优化 README.md 文件，完善结构和内容，增加仓库定位、使用说明等部分
- **2026-04-06**：更新优化 README.md 文件，完善目录索引和内容结构
- **2026-04-05**：补充路径说明、与 Git 分册的关联及双语标题
- **2026-04-05**：初版发布，建立十篇子文档与索引结构
