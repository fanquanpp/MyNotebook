...
Flexbox（弹性盒模型）是CSS3中引入的一种新的布局模式，它提供了一种更灵活、更强大的方式来布局、对齐和分配容器中项目的空间，即使它们的大小是未知的或动态的。
...
### 2.1 Flex容器与Flex项目
- **Flex容器**：应用了`display: flex`或`display: inline-flex`的元素
- **Flex项目**：Flex容器的直接子元素
...
.item {
  ...
  /* 或 */
  flex: 2 0 200px; /* 放大比例2，不缩小，初始宽度200px */
}
```
...
### 6.1 浏览器支持
- **现代浏览器**：Chrome, Firefox, Safari, Edge 都支持Flexbox
- **IE11**：部分支持，需要使用旧的语法
...
### 7.1 性能优化
- **避免嵌套Flex容器**：过多的嵌套会影响性能
- **合理使用flex属性**：避免过度使用flex-grow和flex-shrink
- **使用简写属性**：使用flex简写属性减少代码量
...
**问题2：Flex项目溢出**
- **解决方案**：设置适当的flex-basis，或使用overflow属性
...
Flexbox是一种强大的布局技术，它提供了一种更灵活、更直观的方式来布局网页元素。通过掌握Flexbox的核心概念和属性，你可以创建出更加复杂、响应式的布局。

### 9.1 核心要点
- **Flex容器**：应用`display: flex`的元素
- **Flex项目**：Flex容器的直接子元素
- **主轴与交叉轴**：Flex布局的两个方向
- **容器属性**：控制容器内项目的整体布局
- **项目属性**：控制单个项目的行为
...
通过不断实践和应用，你将能够熟练使用Flexbox，创建出更加美观、灵活的网页布局。