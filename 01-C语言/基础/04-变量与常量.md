...
2. 以下关于全局变量的说法，正确的是：
   A. 全局变量未初始化时值为随机
   B. 全局变量的作用域仅限于定义它的函数
   C. 全局变量在程序开始时创建，结束时销毁
   D. 全局变量不能被其他文件访问
   **答案: C**

3. 以下关于 `static` 变量的说法，正确的是：
   A. 静态局部变量在函数结束后会被销毁
   B. 静态全局变量可以被其他文件访问
   C. 静态变量未初始化时值为 0
   D. 静态变量的作用域是全局的
...
4. 以下关于 `const` 常量和 `#define` 宏的区别，错误的是：
   A. `const` 常量有类型检查，`#define` 宏没有
   B. `const` 常量占用内存，`#define` 宏不占用
   C. `const` 常量的作用域是全局的，`#define` 宏的作用域是块级的
   D. `const` 常量可以在调试器中查看，`#define` 宏不能
   **答案: C**

### 7.2 填空题
1. 变量的存储类包括 `auto`、`static`、`extern` 和 ________。
   **答案: register**

2. 全局变量和静态变量未初始化时，默认值为 ________。
   **答案: 0**

3. 局部变量的默认存储类是 ________。
   **答案: auto**

4. 枚举常量的默认值从 ________ 开始。
   **答案: 0**
...
1. 简述变量的作用域类型及其特点。
   **答案**: 变量的作用域类型包括：
   ...
   - 全局变量：定义在所有函数之外，在整个程序的生命周期内有效
   - 形式参数：函数定义中的参数，作用域仅限于函数内部

2. 简述 `const` 常量和 `#define` 宏的区别。
   **答案**: `const` 常量和 `#define` 宏的区别：
   - 类型检查：`const` 常量有类型检查，`#define` 宏没有
   - 内存占用：`const` 常量占用内存，`#define` 宏不占用（文本替换）
   - 作用域：`const` 常量的作用域是块级，`#define` 宏的作用域是全局（从定义处到文件结束）
   - 调试：`const` 常量可在调试器中查看，`#define` 宏无法查看（已被替换）

3. 什么是左值和右值？请举例说明。
   **答案**: 左值是指向内存位置的表达式，可以出现在赋值语句的左侧；右值是存储在内存中某个地址的数值，只能出现在赋值语句的右侧。例如：`int x = 10;` 中，`x` 是左值，`10` 是右值。

### 7.4 编程题
1. 编写一个程序，使用静态变量实现计数器功能，每次调用函数时计数器加1并输出当前值。
...
2. 编写一个程序，使用枚举定义星期几，并根据输入的数字输出对应的星期几名称。
