# Java 语言教程 | Java Programming Tutorial

> @Author: fanquanpp
> @Version: v3.5.0
> @Category: Java Overview
> @Description: Java 完整教程：快速入门、概述与环境、程序结构与语法、数据类型、变量、运算符、控制流、方法、数组、面向对象、集合框架、多线程、IO流、JVM、泛型、Swing、SpringBoot、SpringCloud。| Complete Java tutorial covering quick start, overview & setup, program structure & syntax, data types, variables, operators, control flow, methods, arrays, OOP, collections, multithreading, IO, JVM, generics, Swing, SpringBoot, SpringCloud.

> **路径**：`04-Java/` | **Java**：跨平台、面向对象、强类型的编程语言，广泛应用于企业级开发、Android、大数据等领域。

## 1. 简介 | Introduction

Java 是由 Sun Microsystems（现已被 Oracle 收购）于 1995 年推出的高级编程语言。其"一次编写，到处运行"（Write Once, Run Anywhere）的特性使其成为企业级应用开发、Android 应用开发和大数据处理等领域的主流选择。

## 2. 目录索引 | Directory Index

### 快速查阅 | Quick Reference

- [V04_101-Java名词注释查阅表.md](./V04_101-Java名词注释查阅表.md)

### 基础篇 | Basics

- [C04_100-快速入门.md](./C04_100-快速入门.md)
- [C04_101-概述与环境.md](./C04_101-概述与环境.md)
- [C04_102-程序结构与基础语法.md](./C04_102-程序结构与基础语法.md)
- [C04_103-数据类型与类型转换.md](./C04_103-数据类型与类型转换.md)
- [C04_104-变量与常量.md](./C04_104-变量与常量.md)
- [C04_105-运算符与表达式.md](./C04_105-运算符与表达式.md)
- [C04_106-控制流.md](./C04_106-控制流.md)
- [C04_107-方法与递归.md](./C04_107-方法与递归.md)
- [C04_108-数组与多维数组.md](./C04_108-数组与多维数组.md)
- [C04_109-面向对象基础.md](./C04_109-面向对象基础.md)
- [C04_110-抽象类与接口.md](./C04_110-抽象类与接口.md)
- [C04_111-异常处理.md](./C04_111-异常处理.md)
- [C04_112-集合框架.md](./C04_112-集合框架.md)
- [C04_113-多线程.md](./C04_113-多线程.md)
- [C04_114-IO流.md](./C04_114-IO流.md)
- [C04_115-JVM内存模型.md](./C04_115-JVM内存模型.md)
- [C04_116-泛型.md](./C04_116-泛型.md)
- [C04_117-Swing图形界面.md](./C04_117-Swing图形界面.md)

### 进阶篇 | Advanced

- [G04_201-SpringBoot核心实战.md](./G04_201-SpringBoot核心实战.md)
- [G04_202-SpringCloud微服务.md](./G04_202-SpringCloud微服务.md)

## 3. 更新日志 | Changelog

- **2026-05-03**
  - 修复编码问题，确保 UTF-8 无 BOM 格式
  - 统一 README 结构和版本号

- **2026-04-18**
  - 完成 Java 模块 3.0 结构优化规划，统一文件命名规范，优化目录结构

- **2026-04-06**
  - 深度优化 README.md 文件，完善结构和内容

- **2026-04-05**
  - 初版发布，建立子文档与索引结构
