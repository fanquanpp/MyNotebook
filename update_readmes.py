...
def update_readme(folder_name, config):
    ...
    if glossary_link in content:
        print(f'跳过：{folder_name} 已包含专有名词链接')
        return
    
    if '## 2. 目录索引' in content:
        insert_pos = content.find('## 2. 目录索引')
        insert_pos = content.find('\n- [', insert_pos)
        if insert_pos != -1:
            content = content[:insert_pos] + glossary_link + '\n' + content[insert_pos:]
    elif '## 目录索引' in content:
        insert_pos = content.find('## 目录索引')
        insert_pos = content.find('\n- [', insert_pos)
        if insert_pos != -1:
            content = content[:insert_pos] + glossary_link + '\n' + content[insert_pos:]
    elif '### 基础篇' in content:
        insert_pos = content.find('### 基础篇')
        content = content[:insert_pos] + glossary_link + '\n\n' + content[insert_pos:]
    elif '## 2.' in content:
        insert_pos = content.find('## 2.')
        content = content[:insert_pos] + glossary_link + '\n\n' + content[insert_pos:]
    else:
        print(f'跳过：{folder_name} 无法找到合适的插入位置')
        return
